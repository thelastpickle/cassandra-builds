...
_main() {
  ...
  case $target in
    ...
    "microbench")
      ...
      ;;
    "test")
      echo $( _list_tests "unit" | _split_tests "${split_chunk}")
      ant testclasslist -Dtest.classlistfile=<( _list_tests "unit" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" || echo "failed $target"
      ;;
    "test-cdc")
      echo $( _list_tests "unit" | _split_tests "${split_chunk}")
      ant testclasslist-cdc -Dtest.classlistfile=<( _list_tests "unit" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" || echo "failed $target"
      ;;
    "test-compression")
      echo $( _list_tests "unit" | _split_tests "${split_chunk}")
      ant testclasslist-compression -Dtest.classlistfile=<( _list_tests "unit" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" || echo "failed $target"
      ;;
    "test-burn")
      echo $( _list_tests "burn" | _split_tests "${split_chunk}")
      ant testclasslist -Dtest.classlistprefix=burn -Dtest.timeout=$(_timeout_for "test.burn.timeout") -Dtest.classlistfile=<( _list_tests "burn" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" || echo "failed $target"
      ;;
    "long-test")
      echo $( _list_tests "long" | _split_tests "${split_chunk}")
      ant testclasslist -Dtest.classlistprefix=long -Dtest.timeout=$(_timeout_for "test.long.timeout") -Dtest.classlistfile=<( _list_tests "long" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" || echo "failed $target"
      ;;
    "jvm-dtest")
      echo $( _list_tests "distributed" | _split_tests "${split_chunk}")
      ant testclasslist -Dtest.classlistprefix=distributed -Dtest.timeout=$(_timeout_for "test.distributed.timeout") -Dtest.classlistfile=<( _list_tests "distributed" | grep -v "upgrade" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" -Dtest.runners=1 || echo "failed $target"
      ;;
    "jvm-dtest-upgrade")
      echo $( _list_tests "distributed" | _split_tests "${split_chunk}")
      _build_all_dtest_jars
      ant testclasslist -Dtest.classlistprefix=distributed -Dtest.timeout=$(_timeout_for "test.distributed.timeout") -Dtest.classlistfile=<( _list_tests "distributed" | grep "upgrade" | _split_tests "${split_chunk}") -Dtmp.dir="${TMP_DIR}" -Dtest.runners=1 || echo "failed $target"
      ;;
    *)
