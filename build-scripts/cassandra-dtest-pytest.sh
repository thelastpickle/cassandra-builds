...
if [ "${DTEST_TARGET}" = "dtest" ]; then
    pytest -vv --log-level="INFO" --use-vnodes --num-tokens=32 --junit-xml=nosetests.xml --junit-prefix=${DTEST_TARGET} -s --cassandra-dir=$CASSANDRA_DIR --skip-resource-intensive-tests --keep-test-dir cql_tracing_test.py::TestCqlTracing::test_tracing_simple 2>&1 | tee -a ${WORKSPACE}/test_stdout.txt
