        ...
        if curl --output /dev/null --silent --head --fail ${latest_url}; then
            ...
            if (( ${latest} > ${latest_saved} )) ; then
                for build_number in $(seq $((${latest_saved}+1)) ${latest}) ; do
                    ...
                    if curl --output /dev/null --silent --head --fail "${main_url}/testReport/" ; then
                        if [[ -z "$(find ci-cassandra.apache.org/job/Cassandra-${pipeline}${job_suffix}/ -name ${build_number})" ]] ; then
